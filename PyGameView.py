...
class PygameView:
    ...
    """
    This class connects program with pygame. It runs Pygame application, set
    screen size, title of the program etc. It connects itself with listeners.
    """
    ...
    def refresh(self, event):
        ...
        """
        This function allows to handle events if the user click on button or
        quit the application. It also allows to refresh data on the screen
        with every clock event. It checks events in eventManager and do
        adequate updates on the screen to every event.
        """
        ...
        if isinstance(event, MoneyTextEvent):
            ...
            self.betsMoneySprites.append(TextSprite(event.text + " $",
                                                    (700, 90), 30,
                                                    (150, 150, 150),
                                                    self.playerSprites))
        ...
        if isinstance(event, MoneyToEquals):
            ...
            for sprite in self.betsSprites:
                ...
                sprite.image = None
            self.moneyText = ""
            self.betsSprites.append(TextSprite(event.text, (700, 50), 30,
                                               (150, 150, 150),
                                               self.playerSprites))
        ...
        if isinstance(event, ShowDownEvent):
            ...
            print('----')
            self.showDownResult(event.players, event.player,
                                event.communityCards, event.cardList)

    def showPreFlopCards(self, players):

        """
        This function shows two cards to every player, but user see only its
        ...
        for i in range(len(players)):
            ...
            CardSprite(player.cards[0], Card.CARDS_POSITION,
                       (POS_LEFT - 85, POS_TOP), Card.PLAYER_CARD, imageSrc,
                       self.playerSprites)
            CardSprite(player.cards[1], Card.CARDS_POSITION,
                       (POS_LEFT + 14, POS_TOP), Card.PLAYER_CARD, imageSrc,
                       self.playerSprites)
            TextSprite(player.name, (POS_LEFT - 20, POS_TOP - 80), 30,
                       (150, 150, 150), self.playerSprites)
    ...
    def showCommunityCards(self, cardList, players):
        ...
        for player in players:
            ...
            if player.currentAlive == 'Alive':
                self.scoreSprites.append(TextSprite(str(player.currentMoney),
                                         (POS_LEFT + 30, POS_TOP - 80), 30,
                                         (150, 150, 150), self.playerSprites))
                self.scoreSprites.append(TextSprite(str(player.moneyOnTable),
                                         (POS_LEFT + 30, POS_TOP - 50), 30,
                                         (150, 150, 150), self.playerSprites))
            else:
                self.scoreSprites.append(TextSprite(str(player.currentMoney),
                                         (POS_LEFT + 40, POS_TOP - 80), 30,
                                         (150, 150, 150), self.playerSprites))
                self.scoreSprites.append(TextSprite(str(player.moneyOnTable),
                                         (POS_LEFT + 40, POS_TOP - 20), 30,
                                         (150, 150, 150), self.playerSprites))
                self.scoreSprites.append(TextSprite('Poza runda',
                                         (POS_LEFT + 30, POS_TOP - 50), 30,
                                         (150, 150, 150), self.playerSprites))
        i = 0
        for card in cardList:
            i += 1
            CardSprite(card, Card.CARDS_POSITION, (350 + i * 100, 350),
                       Card.COMMUNITY_CARD, '', self.communitySprites)

    def showTurnCard(self, card, players):

        """
        This function shows 1 more community card in the
        same row as the previous cards
        """
        ...
        for player in players:
            ...
            if player.currentAlive == 'Alive':
                ...
                self.scoreSprites.append(TextSprite(str(player.currentMoney),
                                         ...
                                         (150, 150, 150), self.playerSprites))
                self.scoreSprites.append(TextSprite(str(player.moneyOnTable),
                                         (POS_LEFT + 40, POS_TOP - 50), 30,
                                         (150, 150, 150), self.playerSprites))
                self.scoreSprites.append(TextSprite('Poza runda',
                                         (POS_LEFT + 30, POS_TOP - 20), 30,
                                         (150, 150, 150), self.playerSprites))
        CardSprite(card, Card.CARDS_POSITION, (750, 350), Card.COMMUNITY_CARD,
                   '', self.communitySprites)

    def showRiverCard(self, card, players):

        """
        This function shows the fifth community card in the same row as the
        previous cards, this is the last community card
        """
        ...
        for player in players:
            ...
            if player.currentAlive == 'Alive':
                ...
                self.scoreSprites.append(TextSprite('Poza runda',
                                         (POS_LEFT + 30, POS_TOP - 20), 30,
                                         (150, 150, 150), self.playerSprites))
        CardSprite(card, Card.CARDS_POSITION, (850, 350), Card.COMMUNITY_CARD,
                   '', self.communitySprites)

    def showDownResult(self, players, player, communityCards, cardsList):

        """
        This function clear the screen from previous cards and display
        who is the winner and what cards the winner has
        """
        ...
        for playerTemp in players:
            ...
            if playerPos == 0:
                ...
                POS_LEFT = 1150
                POS_TOP = 210
            self.scoreSprites.append(TextSprite(str(playerTemp.currentMoney),
                                     (POS_LEFT + 40, POS_TOP - 80), 30,
                                     (150, 150, 150), self.playerSprites))
        ...
        for card in communityCards:
            i += 1
            CardSprite(card, Card.CARDS_POSITION, (350 + i * 100, 380),
                       Card.COMMUNITY_CARD, '', self.communitySprites)

        i = 0
        for card in cardsList:
            i += 1
            CardSprite(card, Card.CARDS_POSITION, (350 + i * 100, 640),
                       Card.COMMUNITY_CARD, '', self.communitySprites)

        TextSprite("Gre wygrywa " + player.name, (550, 150), 60, (200, 30, 10),
                   self.communitySprites)
        TextSprite("Nacisnij strzalke w prawo aby rozpoczac nowa runde",
                   (550, 200), 60, (150, 150, 150), self.communitySprites)
        if player.roundResult is not None:
            TextSprite("\""+player.roundResult.resultName+"\"", (550, 260), 50,
                       (200, 40, 200), self.communitySprites)
